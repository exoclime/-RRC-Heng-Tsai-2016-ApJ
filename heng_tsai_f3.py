...
n_o_tea = np.append(arange(0.1, 1., 0.1),arange(1,10.1,1.) )  
...
n_o = 5e-4
n_c = 0.5 *n_o
NtoO = np.logspace(-1.,1.,100) # the N/O array
n_n = NtoO*n_o
if plot_fig == '3a':
    temperature = 800.
elif plot_fig == '3b':
    ...
    print 'Please specify plot_fig ("3a","3b")'    

if plot_fig == '3a':
    title = r'$P=1$ bar, $T$ = 800 K, $\tilde{n}_{\rm C}=2.5\times 10^{-4}$, $\tilde{n}_{\rm O}=5\times 10^{-4}$ '
elif plot_fig == '3b':
    title = r'$P=1$ bar, $T$ = 1500 K, $\tilde{n}_{\rm N}=2.5\times 10^{-4}$, $\tilde{n}_{\rm O}=5\times 10^{-4}$'
...
for i in range(0,nt):
    n_co[i] = n_cmono(n_o,n_c,n_n[i],temperature,pbar)
    n_co_old[i] = n_cmono_old(n_o,n_c,temperature,pbar)
    n_h2o[i] = n_water(n_o,n_c,n_n[i],temperature,pbar)
    n_h2o_old[i] = n_water_old(n_o,n_c,temperature,pbar)
    n_ch4[i] = n_methane(n_o,n_c,n_n[i],temperature,pbar)
    n_ch4_old[i] = n_methane_old(n_o,n_c,temperature,pbar)
    n_nh3[i] = n_ammonia(n_o,n_c,n_n[i],temperature,pbar)
    n_n2[i] = n_nitrogen(n_o,n_c,n_n[i],temperature,pbar)
    n_hcn[i] = n_hcyan(n_o,n_c,n_n[i],temperature,pbar)
    n_co2[i] = n_cdio(n_o,n_c,n_n[i],temperature,pbar)
    n_c2h2[i] = n_acet(n_o,n_c,n_n[i],temperature,pbar)
    n_c2h2_old[i] = n_acet_old(n_o,n_c,temperature,pbar)
    n_c2h4[i] = n_ethy(n_o,n_c,n_n[i],temperature,pbar)

# PLOT
line1, =plt.plot(NtoO, n_ch4, linewidth=5, color='k', linestyle='--',zorder=0)
line2, =plt.plot(NtoO, n_h2o, linewidth=5, color='m', linestyle=':',zorder=0)
line3, =plt.plot(NtoO, n_co, linewidth=5, color='c', linestyle='-.',zorder=0)
line4, =plt.plot(NtoO, n_nh3, linewidth=5, color='b', linestyle='-',zorder=0)
line5, =plt.plot(NtoO, n_n2, linewidth=5, color='b', linestyle=':',zorder=0)
line6, =plt.plot(NtoO, n_hcn, linewidth=5, color='r', linestyle='-',zorder=0)
line7, =plt.plot(NtoO, n_co2, linewidth=5, color='g', linestyle='--',zorder=0)
line8, =plt.plot(NtoO, n_c2h2, linewidth=5, color='y', linestyle='--',zorder=0)
line9, =plt.plot(NtoO, n_c2h4, linewidth=5, color='y', linestyle=':',zorder=0)
plt.plot(NtoO, n_c2h2_old, linewidth=1, color='y', linestyle='-',zorder=0)
plt.plot(NtoO, n_co_old, linewidth=1, color='c', linestyle='-',zorder=0)
plt.plot(NtoO, n_ch4_old, linewidth=1, color='k', linestyle='-',zorder=0)
plt.plot(NtoO, n_h2o_old, linewidth=1, color='m', linestyle='-',zorder=0)
plt.scatter(n_o_tea, tea[:,ch4_col]/tea[:,h2_col], s=40, facecolor='1.', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,h2o_col]/tea[:,h2_col], s=40, facecolor='m', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,co_col]/tea[:,h2_col], s=40, facecolor='c', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,nh3_col]/tea[:,h2_col], s=40, facecolor='b', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,n2_col]/tea[:,h2_col], s=40, facecolor='b', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,hcn_col]/tea[:,h2_col], s=40, facecolor='r', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,co2_col]/tea[:,h2_col], s=40, facecolor='g', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,c2h2_col]/tea[:,h2_col], s=40, facecolor='y', edgecolors='k',zorder=1)
plt.scatter(n_o_tea, tea[:,c2h4_col]/tea[:,h2_col], s=40, facecolor='y', edgecolors='k',zorder=1)
